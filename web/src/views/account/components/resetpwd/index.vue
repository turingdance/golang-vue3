<template>
  <div class="login-card">
    <div class="box-card content">
      <div class="fm-field"><h3 class="title">重置密码</h3></div>
      <div class="fm-field">
        <input
      ...
      <div class="fm-field row relative">
        <input
          ...
          class="line-input"
        />
        <Smstask class="btn-sms" :mobile="loginForm.username" @error="onerror($event)" />
      </div>

      <div class="fm-field">
        <input
          v-model.trim="loginForm.password"
          placeholder="请设置新的密码,6位以上,数字+字母+特殊字符"
          readonly
          auto-complete="none"
          type="password"
          tabindex="2"
      ...
      <div class="btn-wraper fm-field">
        ...
        <el-link type="primary">点击这里注册账号</el-link>
      </RouterLink>
        <RouterLink to="/login">
      ...
      <div class="fm-field tip">
        <agreement v-model="agreeit"></agreement>
      </div>
    </div>
  </div>
</template>
...
const handleconfirm = (params: any) => {
  ...
  loginForm.captchaCode = dotsA.join(",")
  handleLogin()
}
const handlerefresh = () => {
  createCode()
