<template>
  <div class="wg-cap-wrap">
    <div class="wg-cap-wrap__header">
      ...
      <img class="wg-cap-wrap__thumb" v-if="thumbBase64" :src="thumbBase64" alt=" " />
    </div>
    <div class="wg-cap-wrap__body" :style="style">
      <img class="wg-cap-wrap__picture" v-if="imageBase64" :src="imageBase64" alt=" " @click="handleClickPos($event)" />
      <img
...
const props = defineProps({
  ...
  calcPosType: {
    type: String,
    default: "dom",
    validator: (value) => ["dom", "screen"].includes(value as string)
  },
  maxDot: {
    type: Number,
    default: 5
...
const { imageBase64, thumbBase64, calcPosType } = toRefs(props)
const dots = ref<IDot[]>([])
const handleemit = defineEmits(["close", "refresh", "confirm"])
 ...
 * @param ev
 */
const handleClickPos = (ev: MouseEvent) => {
 ...
 * @param el
 */
const calcLocationLeft = (el: { offsetLeft: any; offsetParent: any }) => {
 ...
 * @param el
 */
const calcLocationTop = (el: { offsetTop: any; offsetParent: any }) => {
