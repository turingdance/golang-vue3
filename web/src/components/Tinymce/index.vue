...
<script lang="ts">
import { defineComponent, computed, onMounted, ref, PropType, unref, watch, onBeforeUnmount } from "vue"
...
import { plugins as initialPlugins, toolbar as initialToolbar, fontFormats } from "./params"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const tinymceId = ref<string>(nanoid())
    const editorRef = ref<Editor>()

    const initOptions = computed((): RawEditorSettings => {
      const publicPath = import.meta.env.VITE_PUBLIC_PATH
      const { height, options, toolbar, plugins, lang, skin,menubar,statusbar } = props
      return {
        menubar,
        statusbar,
        toolbar,
        selector: `#${tinymceId.value}`,
        language_url: `${publicPath}tinymce/langs/${lang}.js`,
        language: lang,
        skin_url: `${publicPath}tinymce/skins/ui/${skin}`,
        skin: skin,
        content_css: `${publicPath}tinymce/skins/ui/${skin}/content.min.css`,
